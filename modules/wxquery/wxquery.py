...
class WxQueryDialog(wx.Dialog):
    def __init__(self,windowtitle, entries,label_size=wx.Size(-1,-1,),control_size=wx.Size(-1,-1)):
        ...
        for entry in entries:
            ...
            if entry[0]=='wxnotebook':
                ...
                self.widgets.append(wx.ComboBox(self.panel,choices=_range.split('|'),value="",size=ctlsz))
                ...
                self.widgets.append(wx.TextCtrl(self.panel,value=str(_default),size=ctlsz))
                self.widgets[-1].SetValue(str(_default))
                self.sizer.Add(wx.StaticText(self.panel,label=_label),1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
                self.sizer.Add(self.widgets[-1], 1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
            elif entry[0]=='wxspin':
                ...
                img = wx.ImageFromStream(StringIO(base64.b64decode(_default)))
                img.Rescale(int(_range.split('|')[0]), int(_range.split('|')[1]))
                ...
                self.widgets.append(wx.ListBox(self.panel,choices=_range.split('|'),size=ctlsz))
                self.widgets[-1].SetSelection(_range.split('|').index(_default))
                self.sizer.Add(wx.StaticText(self.panel,label=_label),1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
                self.sizer.Add(self.widgets[-1], 1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
            elif entry[0]=='wxfile':
                if len(_range)!=0:
                    self.widgets.append(wx.FilePickerCtrl(self.panel,wildcard=_range,size=ctlsz))
                else:
                    self.widgets.append(wx.FilePickerCtrl(self.panel,wildcard=_range,style=wx.FLP_SAVE,size=ctlsz))
                self.widgets[-1].SetPath(_default)
                self.sizer.Add(wx.StaticText(self.panel,label=_label),1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
                self.sizer.Add(self.widgets[-1], 1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
            elif entry[0]=='wxdir':
                self.widgets.append(wx.DirPickerCtrl(self.panel,size=ctlsz))
                self.widgets[-1].SetPath(_default)
                self.sizer.Add(wx.StaticText(self.panel,label=_label),1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
                self.sizer.Add(self.widgets[-1], 1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
            elif entry[0]=='wxchecklist':
                self.widgets.append(wx.CheckListBox(self.panel,choices=_range.split('|'),size=ctlsz))
...
def WxQuery(title, entries,label_size=wx.Size(-1,-1),control_size=wx.Size(-1,-1)):
    if isinstance(control_size,tuple):
        control_size=wx.Size(*control_size)
    if isinstance(label_size,tuple):
        label_size=wx.Size(*control_size)
    dlg = WxQueryDialog(title,entries,label_size,control_size)
    res = dlg.ShowModal()
    values=None
    if res == wx.ID_OK:
...
#see also http://www.ibm.com/developerworks/library/l-script-linux-desktop-2/
    ...
    entries=[('wxnotebook','Entry',None,None,None),                                                 #0
             ...
             ('wxfile','File picker','Acrobat files (*.py)|*.py',"C:\\",'str','extra1',23.154),
    ...
    app = wx.App(False)
    print (WxQuery("A sample dialog",entries,control_size=(250,-1)))
    app.MainLoop()
